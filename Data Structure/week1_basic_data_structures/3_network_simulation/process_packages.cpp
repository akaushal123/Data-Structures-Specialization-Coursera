...
public:
    ...
    Response Process(const Request &request) {
        ...
        //Remove the Request which are processed
        ...
        //if buffer is empty push it to queue and process immediately
        if(finish_time_.empty()){
            finish_time_.push(request.arrival_time + request.process_time);
            return Response(false, request.arrival_time);
        }

        //if there are element in queue then check when will it be start to get processed.
